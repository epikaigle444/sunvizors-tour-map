...
const WelcomeScreen = ({ onStart }) => {
  ...
  useEffect(() => {
    // 1. Vérifier si c'est une invitation via l'URL
    const params = new URLSearchParams(window.location.search);
    const cityParam = params.get('city');
    
    // 2. Vérifier si l'utilisateur a déjà vu l'intro
    const hasSeenIntro = localStorage.getItem('sunvizors_intro_seen');

    if (cityParam) {
      // Si invitation, on affiche TOUJOURS l'intro avec le message spécial
      setInviteCity(cityParam);
      setIsVisible(true);
    } else if (!hasSeenIntro) {
      // Sinon, on affiche seulement si c'est la première visite
      setIsVisible(true);
    } else {
      // Sinon on lance direct
      onStart();
    }
  }, []);
